  ...
  try {
    ...
    if (!response.ok) {
      return NextResponse.json(
        toErrorBody(payload, "Unable to load portfolio."),
...
export async function PATCH(
  ...
  try {
    ...
    const response = await patchBackendJson(`/loan/portfolios/${id}`, body, {
      headers: getAuthHeaders(accessToken),
    });
    ...
    if (!response.ok) {
      return NextResponse.json(
        toErrorBody(responsePayload, "Unable to update portfolio."),
        { status: response.status },
      );
    }
...
export async function DELETE(
  ...
  try {
    ...
    const response = await deleteBackend(`/loan/portfolios/${id}`, {
      headers: getAuthHeaders(accessToken),
    });
    ...
    if (!response.ok) {
      return NextResponse.json(
        toErrorBody(responsePayload, "Unable to delete portfolio."),
        { status: response.status },
      );
    }
