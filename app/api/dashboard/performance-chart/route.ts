...
const forwardedParameters = ["branchId", "loanOfficerId", "recentLimit", "endDate"];
...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await getBackendJson(`/dashboard/performance-chart${query ? `?${query}` : ""}`, {
    ...
    return NextResponse.json(
      { message: "Unable to load dashboard performance chart." },
      { status: 502 },
    );
  }
}